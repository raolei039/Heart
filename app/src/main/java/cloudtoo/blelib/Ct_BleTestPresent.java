...
import com.blelibrary.datahead.GpsDataTest;
 ...
 */
public class Ct_BleTestPresent {
    private Context mContext;
    private static final String TAG = "Ct_BleTestPresent";
    ...
    public Ct_BleTestPresent(Context m)
    {
        this.mContext=m;
        Ct_BindService();
    }
    private Handler SendFileHandler = new Handler()
    {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what)
            {
                case 1000:
                    int pro= (int)msg.obj;
                    if(msendfiledialog!=null && pro>0)
                    {
                        msendfiledialog.setProgress(pro);
                    }
                    if(pro==-1)
                    {

                        String show="成功!耗时：;平均速率:";
                        Ct_ShowAlert(show,0);
                    }
                    break;
                case 1001:
                    Ct_sendfileDestryProgress();
                    int result= (int)msg.obj;
                    String show="错误："+result;
                    Ct_ShowAlert(show,0);
                    break;
                case 1002:
    ...
    private void Ct_BindService()
    {
       ...
       intent.addAction(BleProtocol.ReceMessageHead+BleProtocol.Ble_memory_full);
        //sdk v1.1.9 更新
        intent.addAction(BleProtocol.ReceMessageHead+BleProtocol.Ble_HR_PRV);
        ...
        private void Analysis_Result(Intent intent)
        {
            ...
            //SDK v1.1.1 更新
                ...
                {//内存满
                 ...
                 //sdk v1.1.9 更新
                if(action.equals(BleProtocol.ReceMessageHead+BleProtocol.Ble_HR_PRV))//实时心率中有两个字节是哪里保存prv的值的
                {
                    Object d= intent.getSerializableExtra(BleProtocol.ReceMessage) ;
                    if(d instanceof HeartData)
                    {
                        int prv= ((HeartData) d).prv;
                        Log.e("get prv",prv+" ");
                    }
                    return ;
                }
            ...
            else if(d instanceof GpsDataTest)
                {
                    GpsDataTest result=(GpsDataTest)d;
                    String show=result.gpstestlog;
                    Ct_ShowAlert(show,0);
                    return ;
                }
                Ct_ShowAlert("失败", 0);
    ...
    public void Ct_listItenOnclik(ArrayList<Map<String, Object>>listItem,int position)
    {
            ...
            Log.d(TAG, "Select cmdname :" + cmdname);
            int cmd_index=0;
            for(int i=0;i<BLECMD_NAMELIST.length;i++)
            {
            ...
            Log.d(TAG, "Select cmd_index :" + cmd_index);
        ...
        else if(cmd_index == 56)
            {
                ...
                String starttime= date.format(c.getTime());
               ...
               // if(mapdatabuffer==null)
                {
                    ...
                    try {
                        ...
                        //得到资源中的asset数据流
                        InputStream in = mContext.getResources().getAssets().open("Course2.map");

                        data_total_len = in.available();
                        Log.v("androidsdk", "length:" + data_total_len);
                        mapdatabuffer = new byte[data_total_len];
                        in.read(mapdatabuffer);
                        in.close();
                ...
                Ct_sendfileProgress(starttime,0);
                ...
                getBleService().BLEdata_Initsenfile(new SynMapDataResult() {
                    ...
                    public void BleSynResult(int adress, int datalen, int reslut) {
                        ...
                        //5:未处在升级状态
                        //当为0 才能进行升级
                        Log.v("androidsdk", "reslut:" + reslut);
                        if (reslut == 0x10) {
                            Thread thread = new Thread(new Runnable() {
                                @Override
                                public void run() {
                                    Log.v("androidsdk","data_have_send:"+data_have_send);
                                    if(data_have_send==0)
                                    {
                                        try {
                                            Thread.sleep(8*1000);
                                    ...
                                    data.what = 1000;
                                    float pro= (float)data_have_send/1000;
                                    data.obj = (int)pro;


                                    if (data_have_send + datapackage_len <= data_total_len) {
                                        SendFileHandler.sendMessage(data);
                                        byte[] send = new byte[datapackage_len];
                                        System.arraycopy(mapdatabuffer, data_have_send, send,0 , send.length);
                                        getBleService().BLEdata_Senfile(data_have_send, send);
                                        data_have_send += datapackage_len;

                                    } else {
                                        if (data_have_send >= data_total_len)//结束
                                        {
                                            Log.v("ble", "耗时：" + (System.currentTimeMillis() - testtime) + " 文件大小:" + data_total_len);
                                            getBleService().BLEdata_MapUpdataDone();

                                            Ct_sendfileDestryProgress();
                                             data= new Message();
                                            data.what = 1002;
                                            float speed=(float)1.0*data_total_len/(System.currentTimeMillis()-testtime);
                                            String resutl ="耗时："+(System.currentTimeMillis()-testtime)+
                                                    " 文件大小:" + data_total_len+" 平均速度:"+speed+" kb/s";
                                            data.obj = (String)resutl;
                                            if(mapdatabuffer!=null)
                                            SendFileHandler.sendMessage(data);
                                            mapdatabuffer = null;
                                        } else //最后一包小于1024个字节
                                        {
                                            SendFileHandler.sendMessage(data);
                                            int len = data_total_len - data_have_send;
                                            byte[] send = new byte[len];
                                            System.arraycopy(send, 0, mapdatabuffer, data_have_send, len);
                                            getBleService().BLEdata_Senfile(data_total_len, send);
                                            data_have_send += datapackage_len;
                                        }
                                    }
                                }
            ...
            else if(cmd_index == 57)
            {
                getBleService().nonBLEdata_SetScreenLightTime(0xffffffff);
            }
            else if(cmd_index == 59)
            {
                getBleService().nonBLEdata_SetScreenLightTime(0x0a);
            }
              else if(cmd_index == 103)//开启实时心率
            {
                getBleService().BLEdata_SetHearRealtime(true);
            }else if(cmd_index == 104)//关闭实时心率
            {
                getBleService().BLEdata_SetHearRealtime(false);
            }
             else if(cmd_index == 24)
            {
                Log.e("获取心率","---------------start-----------------");
                  getBleService().BLEdata_GetHeart();
            }else if(cmd_index == 25)
            {
                    getBleService().BLEdata_DelHeart();
            }
    }
    long testtime;
   // private sendatathread   Thread
    byte [] mapdatabuffer;
    int test_index=0;
    int data_have_send=0,data_total_len;
    int datapackage_len =4096;
    ...
    private ProgressDialog msendfiledialog;
    private void Ct_sendfileProgress(String message,int type)
    {
        if(msendfiledialog==null)
            msendfiledialog = new ProgressDialog(mContext);
        //设置进度条风格，风格为圆形，旋转的
        msendfiledialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
        //设置ProgressDialog 标题
        msendfiledialog.setTitle("Cloudtoo");
        //设置ProgressDialog 提示信息
        msendfiledialog.setMessage(message);
        //设置ProgressDialog 标题图标
        msendfiledialog.setIcon(android.R.drawable.ic_dialog_map);
        msendfiledialog.setMax(data_total_len / 1000);
        msendfiledialog.setProgressNumberFormat("%1d kb/%2d kb");
        //设置ProgressDialog 的一个Button
        if(type==1)
            msendfiledialog.setButton("取消", new ProgressDialog.OnClickListener(){
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    msendfiledialog.dismiss();
                }
            });
        //设置ProgressDialog 的进度条是否不明确
        msendfiledialog.setIndeterminate(false);
        //设置ProgressDialog 是否可以按退回按键取消
        msendfiledialog.setCancelable(true);
        msendfiledialog.setCanceledOnTouchOutside(false);
        //显示
        msendfiledialog.show();
        msendfiledialog.setOnCancelListener(new DialogInterface.OnCancelListener()
        {
            @Override
            public void onCancel(DialogInterface dialog) {
                getBleService().BLEdata_MapUpdataCancel();
                msendfiledialog=null;
            }
        });
    }
    private void Ct_sendfileDestryProgress()
    {
        if(msendfiledialog!=null)
        {
            msendfiledialog.dismiss();
